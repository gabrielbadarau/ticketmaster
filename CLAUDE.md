...
- **Switch JWT signing from symmetric to asymmetric (RS256)** — Auth Service holds a private key and signs with it; Booking Service (and any future validating service) holds only the corresponding public key, so a compromised validating service can never forge tokens. Currently both services share one HMAC secret via User Secrets (`Auth:JwtSigningKey`), which doesn't have that property. Consider a JWKS endpoint (`/.well-known/jwks.json`) on Auth Service as a further step, so validating services fetch/cache the public key instead of it being manually copied into config — enables rotation without touching every service by hand.
