...
4. **API Gateway** — routing done via YARP, verified end-to-end (all four route groups proxy correctly to the right service). Rate limiting done too — global, per-client-IP fixed-window limiter (`Microsoft.AspNetCore.RateLimiting`, built into ASP.NET Core since .NET 7, no extra package), `429` + `Retry-After` on rejection, verified end-to-end including the window actually resetting. Auth still deliberately deferred as a separate, larger piece of work.
